...
D8_TARGETS = $(wildcard d8-targets/*.txt)

DISTMAPS = $(patsubst d8-targets/%.txt, distmaps/%.txt, $(D8_TARGETS))
...
distmaps/%.txt: d8-targets/%.txt $(D8S_PATH)/%/cfg cfg_preprocess.py
	mkdir -p distmaps
	./cfg_preprocess.py $(D8S_PATH)/$*/cfg > $@
